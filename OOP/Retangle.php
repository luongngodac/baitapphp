...
<body>
	...
	class HinhTG extends Hinh
	{
		...
		{
			$p = ($this->dodai * 3) / 2;
			return round(sqrt($p * pow(($p - $this->dodai), 3)), 2);
		}
	}
	...
	if (isset($_POST['tinh'])) {
		...
		if (isset($_POST['hinh']) && ($_POST['hinh']) == "tg") {
			...
			$str = "Diện tích của tam giác đều " . $tg->getTen() . " là : " . $tg->tinh_DT() . " \n" .
		...
		<fieldset>
			...
			<table border='0'>
				...
				<tr>
					...
					<input type="radio" name="hinh" value="tg" <?php if (isset($_POST['hinh']) && ($_POST['hinh']) == "tg") echo 'checked="checked"' ?> />Hình tam giác đều</td>
				</tr>
